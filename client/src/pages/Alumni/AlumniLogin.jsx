...
import { Mail, Lock, User, ArrowRight, GraduationCap, Users, Building2, BookOpen, Award, Trophy, Briefcase, Check, AlertCircle, X } from 'lucide-react';
...
function AlumniLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('alumniInfo', JSON.stringify(response.data));
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      await axios.post(`${VITE_API_URL}/forgot-password`, {
        email: forgotEmail
      });
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${VITE_API_URL}/verify-otp`, {
        email: forgotEmail,
        otp: otp
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute w-64 h-64 bg-indigo-600 rounded-full opacity-5 top-[-5rem] left-[-5rem] animate-float"></div>
        <div className="absolute w-96 h-96 bg-indigo-600 rounded-full opacity-5 top-1/4 right-[-8rem] animate-floatReverse"></div>
        <div className="absolute w-96 h-96 bg-indigo-600 rounded-full opacity-5 bottom-[-10rem] left-[-10rem] animate-float"></div>
      </div>

      {/* Floating icons */}
      <Trophy className="absolute left-[15%] top-[20%] text-indigo-600 opacity-10 w-16 h-16 animate-float" />
      <BookOpen className="absolute right-[10%] top-[30%] text-indigo-600 opacity-10 w-20 h-20 animate-floatReverse" />
      <Award className="absolute left-[10%] bottom-[20%] text-indigo-600 opacity-10 w-24 h-24 animate-float" />
      <Briefcase className="absolute right-[15%] bottom-[15%] text-indigo-600 opacity-10 w-16 h-16 animate-floatReverse" />
      ...
      <div className="w-full max-w-6xl flex flex-col lg:flex-row items-center justify-center gap-8 relative z-10">
        {/* Left content - hidden on mobile */}
        <div className="hidden lg:flex flex-col items-center gap-8 w-1/2">
          <GraduationCap className="text-indigo-600 w-24 h-24 animate-float hover:scale-110 transition-transform" />
          <h4 className="text-3xl font-bold text-gray-900 text-center">Welcome to Alumni Network</h4>
          <p className="text-gray-500 text-center text-base max-w-[28rem]">
            Connect with fellow alumni, share experiences, and stay updated with the latest news and events from your alma mater.
          </p>
          
          {/* Stats grid */}
          <div className="grid grid-cols-2 gap-6 w-full max-w-[28rem] mt-4">
            <div className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all animate-scaleIn">
              <Users className="text-indigo-600 w-6 h-6 mb-2" />
              <div className="text-xl font-bold text-gray-900">5000+</div>
              <div className="text-gray-500">Active Alumni</div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all animate-scaleIn">
              <Building2 className="text-indigo-600 w-6 h-6 mb-2" />
              <div className="text-xl font-bold text-gray-900">100+</div>
              <div className="text-gray-500">Partner Companies</div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all animate-scaleIn">
              <Trophy className="text-indigo-600 w-6 h-6 mb-2" />
              <div className="text-xl font-bold text-gray-900">250+</div>
              <div className="text-gray-500">Success Stories</div>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all animate-scaleIn">
              <Award className="text-indigo-600 w-6 h-6 mb-2" />
              <div className="text-xl font-bold text-gray-900">50+</div>
              <div className="text-gray-500">Annual Events</div>
            </div>
          </div>
        </div>
