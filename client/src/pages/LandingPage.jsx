...
function App() {
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${VITE_API_URL}/contact/submit`, formData);
      setSubmitStatus({
        type: 'success',
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const response = await fetch(`${VITE_API_URL}/subscribe`, {
      method: 'POST',
      headers: {
  ...
  // Success stories from backend
    ...
    useEffect(() => {
      const fetchStories = async () => {
        try {
          const response = await fetch(`${VITE_API_URL}/success-stories`);
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <span className={`ml-2 text-2xl font-bold ${scrolled ? 'text-gray-800' : 'text-white'}`}>Network Nexus</span>
              </div>
